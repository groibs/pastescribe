...
### 2026-08-04 — Onda 4 fatia 4.2a (correção): separar criar o job de reservar orçamento

- **Status:** ativa
- **Contexto:** a primeira versão desta fatia (`reserve_free_budget_and_enqueue`) reservava orçamento free e criava o job no mesmo instante, presumindo que o custo estimado (duração × tarifa × 1,5×, `docs/AI_COST_MODEL.md` §3) já dava pra calcular ali. O dono apontou o problema real: pra upload, a duração só é sabida depois que o worker baixa o arquivo e roda `ffprobe`; pra link (YouTube/TikTok/Instagram — ainda sem adapter, mas já pensado aqui), a duração vem de metadata pública da própria plataforma, e antes disso pode até haver **legenda nativa pronta**, que não custa IA nenhuma. Declarar a duração no client (opção descartada explicitamente) não serviria — o cliente pode mentir, e o objetivo é decidir com o dado real, não confiar em mais um "declarado" sem verificação barata disponível (diferente de `content_type`/`size`, que dá pra checar via `headObject` quase instantaneamente; duração exige abrir o arquivo).
- **Decisão:** `enqueue_job` cria o job (`'queued'`) sem nenhuma reserva de orçamento — é trabalho determinístico e grátis (skill `pastescribe-ai-cost-governance`: "metadados via API/ffprobe" nunca é cobrado como IA). O worker processa o passo determinístico que revela a duração real (`acquiring_media`→ffprobe pra upload; `resolving_metadata` pra link) e só então chama `reserve_job_budget`, que grava `duration_seconds` e decide: cabe no free → reserva de verdade, job segue pra `transcribing`; excede o teto → job vai pro estado `awaiting_user_confirmation` (já existia na máquina de estados desde a Onda 0, sem nenhum caminho real que o alcançasse até agora), sem cobrar ninguém — não existe checkout pago ainda (Onda 9) pra virar cobrança de fato. `source_kind` ganhou o valor `'url'` estruturalmente (coluna `source_url`, check constraint) nesta mesma correção, para não precisar de outra migration de schema quando a Onda 8 chegar — mas nenhum adapter, rota, SSRF ou normalização de URL foi construído; nenhuma rota real cria job `source_kind='url'` hoje.
- **Custo de descobrir a duração:** não passa pelo orçamento de IA (`free_ai`) — é CPU/banda do próprio worker (upload) ou uma chamada a uma API de metadata geralmente gratuita da plataforma (link), coberto pelo envelope separado "Ingestão/processamento/worker" (R$150/mês). Ainda assim é custo operacional real e abusável (spam de upload/link só pra gastar worker) — por isso a expectativa é que a camada que chamar `enqueue_job` passe por `consume_quota` antes (política de bucket/janela ainda não decidida, mesmo raciocínio já registrado pra outras quotas: decisão de quem chama, não do banco).
- **Consequência:** retomar um job em `awaiting_user_confirmation` depois que o usuário pagar/fizer upgrade é **A confirmar** — nada foi construído pra isso (o estado já permite voltar pra `queued`, mas re-enfileirar do zero refaria acquiring_media/extracting_audio à toa; a forma certa de retomar sem redundância fica pra quando existir um fluxo pago de verdade pra testar contra). `reserve_free_budget_and_enqueue` foi removida (`drop function`) — nenhuma migration anterior tinha sido aplicada no projeto Supabase real do dono, então não há dado de produção afetado por essa remoção.

### 2026-08-04 — Onda 4 fatia 4.2b: `POST /api/uploads/[id]/complete` enfileira automaticamente

- **Status:** ativa
- **Contexto:** com `enqueue_job` já separada de qualquer custo de orçamento (correção da 4.2a), enfileirar um job deixou de ser uma decisão "cara" — é só criar a linha. Faz sentido acontecer automaticamente assim que o upload é validado, sem exigir um segundo clique do usuário (o produto é "colar link ou subir arquivo → ver o resultado", não uma tela de configuração intermediária).
- **Decisão:** `POST /api/uploads/[id]/complete` chama `enqueue_job` (via admin client, `service_role`) logo depois de marcar o `media_asset` como `validated`. Antes disso, chama `consume_quota` com bucket `enqueue:user:<uuid>`, janela = dia corrente (UTC), limite `MAX_JOBS_ENQUEUED_PER_DAY = 20` (`apps/web/lib/jobs/constants.ts`, provisório — sem política por plano ainda, Onda 9) — protege o passo de enfileirar contra abuso (CPU/banda do worker), não é o gate de orçamento de IA (esse é outro, dentro de `reserve_job_budget`, só quando o worker souber a duração real). Falha em qualquer um dos dois (quota estourada ou `enqueue_job` falhar) não derruba a resposta 200 da validação do upload — o asset já está validado de verdade; o response inclui `job: null` e `jobError` explicando o motivo, em vez de misturar os dois domínios de falha num único status HTTP.
- **Consequência:** este é o primeiro código de aplicação que chama `consume_quota`/`enqueue_job` via `.rpc()` do Supabase JS — não pôde ser exercitado ao vivo neste sandbox (sem projeto Supabase real, mesma limitação de sempre); a tipagem foi verificada via `tsc` contra `Database["public"]["Functions"]` e os dois RPCs já têm cobertura pgTAP profunda a partir da 4.2a. Um `media_asset` validado agora sempre tenta gerar um `transcription_job`, mas nenhum worker existe ainda pra processá-lo (4.2c) — o job fica parado em `queued` até lá, o que é esperado.
